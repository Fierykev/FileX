...
float3 genMarble(float3 origCol, float3 pos)
{
	...
	tmp += (noise0.SampleLevel(linearRepeatSample, pos * freq * .324, 0).xyz * 2.f - 1) * pow(.43, -1);
	tmp += (noise1.SampleLevel(linearRepeatSample, pos * freq * 1.23, 0).xyz * 2.f - 1);
	tmp += (noise2.SampleLevel(linearRepeatSample, pos * freq * 1.83, 0).xyz * 2.f - 1) * pow(.43, 1);
