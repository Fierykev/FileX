...
float ambientOcclusion(float3 position, float3 worldPos)
{
	float vis = 0;

	const float skipCells = 1.25;
	...
	for (uint i = 0; i < 32; i++)
	{
		...
		float3 delta =
			direction * occExpansion.xxx / 160.f / NUM_STEPS;
		...
		for (uint j = 0; j < NUM_STEPS; j++)
		{
			...
			isVis = lerp(isVis, 0, saturate(den));
		}
		
		// LONG RANGE INCORRECT
		for (uint k = 0; k < FAR_SAMPLES; k++)
		{
			...
			float den = density(worldPos + direction * range);
			isVis *=
...
VS_OUTPUT locateVertFromEdge(float3 position, float3 sampleArea, uint edgeNum)
{
	...
	float3 worldCoord = sampleArea + relPos * voxelSize.xxx;
	...
	vout.position.w = ambientOcclusion(uvw, worldCoord);
